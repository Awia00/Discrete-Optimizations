...
public class SubsetOfAtMostSizeIterator<T extends Comparable<T>> implements Iterator<TSP.Set.Set<T>> {
    private final List<T> elements;
    private final Iterator<TSP.Set.Set<T>> inner;
    private final int maxSize;

    private int index;
    private TSP.Set.Set<T> sub;
    private boolean nextTaken = true;
    private TSP.Set.Set<T> next;

    @SuppressWarnings("unchecked")
    public SubsetOfAtMostSizeIterator(TSP.Set.Set<T> elements, int size) {
    ...
    @Override
    public TSP.Set.Set<T> next() {
        if (hasNext()) {
            nextTaken = true;
            TSP.Set.Set<T> next = this.next;
            this.next = null;
            return next;
        }
    ...
    public static class SetIterator<T> implements Iterator<TSP.Set.Set<T>> {
        ...
        @Override
        public TSP.Set.Set<T> next() {
            return TSP.Set.Set.of(inner.next());
        }
    }
}
